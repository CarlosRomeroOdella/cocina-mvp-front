...
export default function ClientMenu() {
  ...
  // Al montar: si el pedido ya estaba en revisión (recarga de página), abrir modal
  useEffect(() => {
  ...
  // Polling — solo depende del ID para evitar que el cambio de status reinicie el intervalo
  // y pierda la detección de "en_revision"
  useEffect(() => {
    if (!pedidoActivo?.id) return;
    const ivRef = { current: null };
    const poll = async () => {
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
              ...
              {!loadingHistorial && errorHistorial && (
                <div className="text-center py-16">
                  ...
                  <button onClick={() => { setHistorial([]); setErrorHistorial(false); abrirHistorial(); }} className="text-xs font-semibold text-orange-500 hover:underline">Reintentar</button>
                </div>
              )}
